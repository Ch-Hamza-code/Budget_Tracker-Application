...
router.post('/signup', async (req, res) => {
    const { firstName, lastName, email, password, confirmPassword, role, budgetLimit } = req.body;
    ...
    try {
        ...
        const newUser = new User({  firstName,  lastName,   email,  password,   confirmPassword,   budgetLimit,role: role || 'user'  });
        await newUser.save();
        res.status(201).json({
...
router.get('/users', async (req, res) => {
    try {
        const users = await User.find({}, '-password -__v'); // Exclude the password field
...
router.get('/me', async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(userId, '-password -__v'); // Exclude password and __v
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }

        // Send the user data (excluding sensitive fields)
        res.status(200).json(user);
    } catch (error) {
